...
?>
    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $check = checkarticle("name", "categories", $name);
            if ($check == 1) {
                ...
                $stmt = $con->prepare("INSERT INTO
                                categories(name, Description, Parent, Ordering, Visibility, Allow_Comment)
                                VALUES(:zuser, :zdesc, :zparent, :zorder, :zvisible, :zcomment)");
                $stmt->execute(array(
                    'zuser'     => $name,
                    'zdesc'     => $desc,
                    'zparent'   => $parent,
...
<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $stmt = $con->prepare("UPDATE categories SET Name = ?, Description = ?, Ordering = ?, Parent = ?, Visibility = ?, Allow_Comment = ? WHERE ID = ?");
            $stmt->execute(array($name, $desc, $order, $parent, $visible, $comment, $id));
        ...
        $catid = isset($_GET['catid']) && is_numeric($_GET['catid']) ? intval($_GET['catid']) : 0;
        // Select All Data Depend On This ID
        $check = checkarticle('ID', 'categories', $catid);
        // If There's Such ID Show The Form
        if ($check > 0) {
